...
export default class Generate extends SfdxCommand {
  ...
  public async run(): Promise<AnyJson> {
    ...
    for (const namedPackageDir of await this.getProjectPackagesToProcess()) {
      ...
      for (const typeInfoDefinition in typeInfos.typeDefs) {
        ...
        for (const contentElement of fs.readdirSync(`${packageSourcePath}/${mtd.defaultDirectory}`, {
          withFileTypes: true,
        })) {
          /**
          ...
          if (mtd.decompositionConfig.workspaceStrategy === WorkspaceStrategy.FolderPerSubtype) {
            ...
            if (fs.fileExistsSync(`${elementpath}/${contentElement.name}.${mtd.ext}-meta.xml`)) {
             ...
             */
            await fs.mkdirp(`${this.flags.outputdir}/${namedPackageDir.name}/${mtd.defaultDirectory}/`);
            // TODO: to parse into markdown here!
            await fs.writeJson(
              `${this.flags.outputdir}/${namedPackageDir.name}/${mtd.defaultDirectory}/${contentElement.name}.json`,
              mtdParsed
            );
            const mdparsedcontent = await jsonToMarkdown(mtdParsed, mtd.metadataName);
            await fs.writeFile(
            ...
            const content = await fs.readFile(`${packageSourcePath}/${mtd.defaultDirectory}/${contentElement.name}`, {
              encoding: 'utf8',
            });
  ...
  private async getProjectPackagesToProcess(): Promise<NamedPackageDir[]> {
    ...
    return this.project.getUniquePackageDirectories().filter((element) => this.flags.packages.includes(element.name));
  }
}
